#!/usr/bin/env luajit
-- Dramatic lap streamer - direct JSON output

io.stderr:write("🏎️  Dramatic Ferrari 488 Pipeline\n\n")
...
while true do
    loop = loop + 1
    io.stderr:write(string.format("🏁 Lap %d\n", loop))
    ...
    while t <= 38 do
        ...
        if frame % 100 == 0 then
            io.stderr:write(string.format(
                "   🏎️ %3.0f km/h | G%d | %5.0f RPM | THR:%3.0f%% | BRK:%2.0f\n",
                state.speed, state.gear, rpm, state.throttle, state.brake
            ))
        end
