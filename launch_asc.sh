...
tmux new-window -t $SESSION:3 -n 'ASC-Stream'
tmux send-keys -t $SESSION:3 'cd ~/project-apex && source setup_env.sh && sleep 3 && luajit examples/run_asc_stream.lua | ros2 run f1_telemetry_publisher can_publisher' C-m

echo "🏎️  F1 ASC Replay System launching..."
sleep 4
